...
def setup_new_online_round(room, category=None):
    ...
    # Generate fresh coordinates for this round
    ...
    # If codes are off, skip ROLE_REVEAL and go straight to PLAYING
...
def local_start_game():
    ...
    # Skip role reveal if codes are disabled
...
def local_verify_code():
    ...
    if use_codes:
        ...
        # Codes are off, use player dropdown index
...
def online_room_state():
    ...
    # --- Send device players (excluding those who voted) for the shared dropdown UI ---
    if room['phase'] in ['VOTING', 'PLAYING']:
        device_players = []
        for dev in room['devices']:
            if player_id in dev:
                for pid in dev:
                    # Only include if they haven't voted yet (during voting phase)
...
def online_verify_code():
    ...
    use_codes = room.get('use_codes', True)
    player_id = session.get('player_id')  # Get the current player

    if use_codes:
        code = str(data.get('code', '')).strip()
        # Get the code assigned to this player
        ...
        # Codes off: just return the target for the selected player
...
let lastTopic = '';   // track topic changes
...
function pollOnlineRoom() {
    ...
    fetch(`/online/room_state?room=${currentRoom}`)
        ...
        .then(data => {
            ...
            // --- DETECT TOPIC CHANGE AND RESET UI ---
            ...
            // Filter unvoted players for this device
...
function renderLobby(data) {
    ...
    // --- Update player list (only if changed) ---
    ...
    if (isHost) {
        ...
        // --- Manual player input (persistent, not recreated) ---
        ...
        // --- Device assignment area (rebuilt only if devices or players changed) ---
        const currentDevices = data.devices || [];
        if (currentDevices.length === 0) currentDevices.push([]);
        
        // Build a signature of the current assignment state
        ...
        if (deviceArea.dataset.sig !== sig) {
            ...
            // Build the device assignment UI
...
function renderPlaying(data) {
    ...
    // Rebuild grid with current highlight (null if topic changed)
...
function renderVoting(data) {
    ...
    if (unvoted.length === 0) {
        ...
        // Show dropdown only if shared and more than 1 unvoted
        ...
        // Build vote buttons for all players
        let html = '';
        data.players.forEach(p => {
        ...
        // Disable buttons for already voted players
